...
def user_sign_up(request):
    if request.method =="POST":
        ...
        if User.objects.filter(email=email).exists():  # Check if the email is already in use
...
def user_log_sign_page(request):
    if request.method == 'POST':
        credential = request.POST['email']  # This can be either username or email
        ...
        if is_email:
            # Credential is an email, so get the username associated with it
            try:
            ...
            # Credential is a username, so directly authenticate
            user = authenticate(username=credential, password=password)

        if user is not None:
            if user.is_active:
                if user.is_staff:
                    # If the user is staff, they should use a different login page
    ...
    # GET request, render the login/sign up page
